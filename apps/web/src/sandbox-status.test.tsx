...
describe("Administrator Sandbox Status surface", () => {
  ...
  it("shows Sandbox Capability availability, Agent authorization, recent calls, and artifact capture", async () => {
    ...
    expect(await within(runtimeStatus).findByText("SandboxAgent")).toBeInTheDocument();
    expect(within(runtimeStatus).getByText("1 个产物")).toBeInTheDocument();

    const capabilityTable = screen.getByRole("table", { name: "智能体沙箱能力" });
    expect(await within(capabilityTable).findByRole("row", { name: /Default Agent 默认启用 由运行审计记录产物 过程可见性：标准/ })).toBeInTheDocument();
    expect(within(capabilityTable).getByRole("row", { name: /Research Agent 默认启用 由运行审计记录产物 过程可见性：最小/ })).toBeInTheDocument();
