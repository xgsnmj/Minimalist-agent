...
describe("MVP smoke workflow surface", () => {
  it("keeps the primary conversation, artifact, cancellation, and admin audit entry points visible", () => {
    ...
    expect(within(screen.getByLabelText("制品预览")).getByRole("heading", { name: "简报" })).toBeInTheDocument();
    expect(within(screen.getByLabelText("制品预览")).getByText("alpha")).toBeInTheDocument();
  });
});
