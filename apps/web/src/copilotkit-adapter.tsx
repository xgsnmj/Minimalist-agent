  ...
  return (
    ...
    >
      ...
      <CopilotSidebar
        ...
        width={380}
        labels={{
          chatInputPlaceholder: "Ask the copilot about this workspace...",
          modalHeaderTitle: "Minimalist Agent Copilot",
          welcomeMessageText:
            "I can help you navigate conversations, stage drafts, and inspect artifacts without bypassing platform governance.",
        }}
      />
    </CopilotKit>
  );
...
export function CopilotWorkspaceBridge({
  ...
  const workspaceContext = useMemo(
    () => ({
      ...
      attachmentPreviewName,
      currentPage: "Conversation Workspace",
      draftAgentId,
      draftModelId,
      lastSeenSequence,
      previewArtifactId,
  ...
  useFrontendTool(
    {
      ...
      handler: async ({ conversationId }) => {
        ...
        if (!conversation) {
          return `Agent Conversation ${conversationId} was not found.`;
        }
        ...
        return `Opened Agent Conversation ${conversation.title}.`;
      },
    },
    [conversations],
  );
  ...
  useFrontendTool(
    {
      ...
      description:
        "Switch to a new Agent Conversation draft and optionally prefill the composer. This does not submit a message.",
      parameters: draftMessageSchema.partial(),
      followUp: false,
      handler: async ({ message }) => {
        ...
        return "Prepared a new Agent Conversation draft.";
      },
    },
    [],
  );
  ...
  useFrontendTool(
    {
      name: "draftConversationMessage",
      description:
        "Prefill the composer for the current or requested Agent Conversation without sending it.",
      parameters: draftMessageSchema,
      followUp: false,
      handler: async ({ conversationId, message }) => {
        if (conversationId) {
          const conversation = conversations.find((item) => item.id === conversationId);
          if (!conversation) {
            return `Agent Conversation ${conversationId} was not found.`;
        ...
        setComposerValue(message);
        return "Drafted the composer message. The user still needs to send it.";
      },
    },
    [conversations],
  );
  ...
  useFrontendTool(
    {
      ...
      handler: async ({ artifactId }) => {
        ...
        if (!conversation) {
          return `Artifact ${artifactId} was not found in visible conversations.`;
        }
        ...
        return `Opened Artifact Preview ${artifactId}.`;
      },
    },
    [conversations],
  );
